...
function changeColor() {
    ...
    if (document.getElementsByTagName('html')[0].getAttribute('data-theme') == 'dark') {
        const siteNameEl = document.getElementsByClassName("site-name")[0];
        if (siteNameEl) {
            siteNameEl.style.textShadow = arr[idx] + " 0 0 15px";
        }
        ...
        const siteNameEl = document.getElementsByClassName("site-name")[0];
        if (siteNameEl) {
            siteNameEl.style.textShadow = "#1e1e1ee0 1px 1px 1px";
        }
        if (document.getElementById("site-name"))
